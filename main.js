...
function generateNotes () {
	var string = '';
	for (var i = 0; i < notesdata.length; i++) {
		if (filter.value.length == 0 || notesdata[i].title.toLowerCase().includes(filter.value.toLowerCase())) {
			string += '<div id="note' + i +  '" class="note">' +
					'<div class="noteupper">' +
						...
						'</span>' +
					'</div>' +
					'<div><textarea id="notecontent' + i + '" class="notecontent" style="height: ' + notesdata[i].height + '" onkeyup="updateNote(' + i + ')" oninput="updateNote(' + i + ')">' + notesdata[i].content + '</textarea></div>' +
				'</div>';
		}
	}
